...
if (!$kamp_id) { header('Location: kelola_kampanye.php'); exit(); }

// Ambil kampanye — pastikan milik pengelola ini
$kamp = mysqli_fetch_assoc(mysqli_query($koneksi,
    "SELECT * FROM kampanye WHERE id=$kamp_id AND pengelola_id=$user_id LIMIT 1"));
if (!$kamp) { header('Location: kelola_kampanye.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
    ...
    if (empty($errors)) {
        $j = esc($judul); $c = esc($cerita); $tg = esc($tag);
        $l = esc($lokasi); $dl_e = esc($dl); $f = esc($foto_path);
...
if (isset($_GET['verify'], $_GET['did'])) {
    ...
    // Ambil nominal
    $don = mysqli_fetch_assoc(mysqli_query($koneksi,
    ...
    header("Location: detail_kelola.php?id=$kamp_id&msg=verified"); exit();
}

// ── HANDLE GET: Hapus kampanye ─────────────────────────────
if (isset($_GET['action']) && $_GET['action'] === 'delete') {
    if ((int)$kamp['terkumpul'] >= 10000) {
        header("Location: detail_kelola.php?id=$kamp_id&msg=cannot_delete"); exit();
    }
...
$backUrl = 'kelola_kampanye.php'; $pageTitle = 'Kelola Kampanye';
...
<head>
    ...
    <style>
        ...
        .pg { max-width:1000px; margin:0 auto; padding:28px 20px 70px; }
        ...
        /* Hero */
        .camp-hero { position:relative; height:220px; border-radius:var(--radius-xl); overflow:hidden; margin-bottom:22px; box-shadow:var(--shadow-md); animation:fadeInUp .4s ease; }
        .camp-hero img { width:100%; height:100%; object-fit:cover; filter:brightness(.68); }
        .camp-hero::after { content:''; position:absolute; inset:0; background:linear-gradient(to bottom,rgba(0,0,0,.05) 30%,rgba(0,0,0,.60) 100%); }
        .camp-hero-body { position:absolute; bottom:20px; left:22px; right:22px; z-index:1; }
        .camp-hero-body h2 { font-family:'Montserrat',sans-serif; font-size:19px; font-weight:900; font-style:italic; color:white; text-shadow:0 2px 8px rgba(0,0,0,.4); margin-bottom:8px; }
        .hbadges { display:flex; gap:8px; flex-wrap:wrap; }
        .hb { background:rgba(255,255,255,.18); backdrop-filter:blur(4px); border:1px solid rgba(255,255,255,.4); color:white; font-size:11px; font-weight:700; padding:3px 10px; border-radius:20px; }
        ...
        .sc-title { font-family:'Montserrat',sans-serif; font-size:14px; font-weight:800; color:var(--green-primary); margin-bottom:14px; padding-bottom:10px; border-bottom:1.5px solid var(--gray-border); display:flex; align-items:center; gap:7px; }

        /* Progress */
        .prog-amt { font-size:22px; font-weight:800; color:var(--green-primary); }
        .prog-target { font-size:13px; color:var(--gray-muted); font-style:italic; margin-bottom:10px; }
        .prog-pct { font-size:11px; font-weight:700; color:var(--green-primary); text-align:right; margin-top:4px; margin-bottom:16px; }

        /* Summary boxes */
        .sum-grid { display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; }
        .sum-box { border-radius:10px; padding:11px 12px; }
        .sum-box.green { background:var(--green-light); }
        .sum-box.yellow { background:#fef9c3; }
        .sum-box.red { background:#fee2e2; }
        .sum-lbl { font-size:10px; font-weight:800; text-transform:uppercase; letter-spacing:.3px; margin-bottom:3px; }
        .sum-box.green .sum-lbl { color:var(--green-primary); }
        .sum-box.yellow .sum-lbl { color:#b45309; }
        .sum-box.red .sum-lbl { color:#dc2626; }
        .sum-val { font-size:13px; font-weight:800; color:#1a2319; }

        /* Donasi list */
        .don-item { display:flex; align-items:flex-start; gap:10px; padding:12px 0; border-bottom:1px solid var(--gray-border); }
        .don-item:last-child { border-bottom:none; }
        .don-av { width:36px; height:36px; border-radius:50%; object-fit:cover; flex-shrink:0; }
        .don-body { flex:1; min-width:0; }
        .don-name { font-weight:700; font-size:13px; color:#1a2319; }
        .don-email { font-size:11px; color:var(--gray-muted); }
        .don-pesan { font-size:12px; color:var(--gray-text); font-style:italic; margin-top:2px; }
        .don-right { text-align:right; flex-shrink:0; }
        .don-nom { font-weight:800; font-size:13px; color:var(--green-primary); }
        .don-date { font-size:10px; color:var(--gray-muted); margin-bottom:5px; }
        .don-actions { display:flex; gap:5px; justify-content:flex-end; margin-top:5px; }
        .don-actions a, .don-actions button {
            padding:4px 10px; border-radius:6px; border:none;
            font-size:11px; font-weight:700; cursor:pointer;
            text-decoration:none; transition:var(--transition);
            font-family:'Poppins',sans-serif;
        }
        .btn-acc { background:#dcfce7; color:#15803d; }
        .btn-acc:hover { background:#15803d; color:white; }
        .btn-rej { background:#fee2e2; color:#dc2626; }
        .btn-rej:hover { background:#dc2626; color:white; }
        .btn-bukti-sm { background:#e0f0ff; color:#2563eb; }
        .btn-bukti-sm:hover { background:#2563eb; color:white; }

        /* Status dot */
        .sdot { display:inline-flex; align-items:center; gap:3px; font-size:11px; font-weight:700; padding:2px 8px; border-radius:20px; }
        ...
        .fg label { display:block; font-size:11px; font-weight:700; color:var(--green-primary); text-transform:uppercase; letter-spacing:.3px; margin-bottom:5px; }
        ...
        /* Danger zone */
        .danger-zone { background:#fff5f5; border:1.5px solid #fecaca; border-radius:var(--radius-md); padding:18px 22px; }
        .danger-zone h4 { color:#dc2626; font-size:13px; font-weight:800; margin-bottom:5px; }
        .danger-zone p { font-size:12px; color:#9a3535; margin-bottom:12px; }

        /* Modal bukti */
        .modal-ov { display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:400; align-items:center; justify-content:center; padding:20px; }
        .modal-ov.show { display:flex; }
        .modal-box { background:white; border-radius:var(--radius-lg); padding:24px; max-width:460px; width:100%; box-shadow:var(--shadow-lg); animation:fadeInUp .3s ease; }
        .modal-box h3 { font-size:15px; font-weight:800; color:var(--green-primary); margin-bottom:12px; }
        .modal-box img { width:100%; border-radius:10px; max-height:340px; object-fit:contain; }

        /* err list */
        .err-list { background:#fee2e2; border-left:4px solid #dc2626; border-radius:var(--radius-sm); padding:11px 16px; margin-bottom:14px; }
        .err-list li { font-size:12px; color:#dc2626; font-weight:600; list-style:none; margin-bottom:2px; }
    </style>
</head>
<body>
