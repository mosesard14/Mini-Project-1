...
if (!$kamp_id) { header('Location: main.php'); exit(); }
...
if (!$kamp) { header('Location: main.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        mysqli_query($koneksi,
            "INSERT INTO donasi (kampanye_id,donatur_id,nominal,metode,pesan,bukti_path,status)
             VALUES ($kamp_id,$user_id,$nom_esc,'$met_esc','$pesan_esc','$bukti_esc','pending')");
        $success = true;
    }
}
...
<head>
    ...
    <style>
        ...
        .kamp-card { background:white; border-radius:var(--radius-md); padding:20px 24px; margin-bottom:24px; box-shadow:var(--shadow-sm); animation:fadeInUp .4s ease; }
        .kamp-row { display:flex; align-items:center; gap:16px; }
        .kamp-img { width:90px; height:70px; border-radius:10px; object-fit:cover; flex-shrink:0; }
        .kamp-judul { font-weight:800; font-size:15px; color:#1a2319; margin-bottom:4px; }
        .kamp-stats { display:flex; gap:24px; margin-top:8px; }
        .ks-lbl { font-size:10px; font-weight:700; color:var(--gray-muted); font-style:italic; }
        .ks-val { font-size:14px; font-weight:800; color:var(--green-primary); }
        ...
        /* anon toggle */
        .anon-row { display:flex; align-items:center; gap:10px; }
        .anon-row input[type="text"] { flex:1; }
        .toggle-lbl { display:flex; align-items:center; gap:6px; font-size:12px; font-weight:600; color:var(--gray-muted); cursor:pointer; white-space:nowrap; }
        .sw { position:relative; display:inline-block; width:36px; height:20px; }
        .sw input { opacity:0; width:0; height:0; }
        .sl { position:absolute; cursor:pointer; top:0;left:0;right:0;bottom:0; background:#ccc; border-radius:20px; transition:.3s; }
        ...
        input:checked+.sl { background:var(--green-primary); }
        input:checked+.sl::before { transform:translateX(16px); }

        /* file drop */
        .file-drop { border:2px dashed var(--gray-border); border-radius:var(--radius-sm); padding:22px; text-align:center; cursor:pointer; transition:var(--transition); color:var(--gray-muted); font-size:13px; }
        .file-drop:hover { border-color:var(--green-primary); color:var(--green-primary); }
        ...
        /* error */
        .err-list { background:#fee2e2; border-left:4px solid #dc2626; border-radius:var(--radius-sm); padding:12px 16px; margin-bottom:20px; }
        .err-list li { font-size:13px; color:#dc2626; font-weight:600; list-style:none; margin-bottom:3px; }

        /* btn submit */
        .btn-donasi { width:100%; padding:14px; background:var(--green-primary); color:white; border:none; border-radius:var(--radius-sm); font-family:'Montserrat',sans-serif; font-weight:800; font-size:16px; cursor:pointer; transition:var(--transition); box-shadow:0 3px 14px rgba(45,138,82,.28); }
        .btn-donasi:hover { background:var(--green-dark); transform:translateY(-1px); }

        /* user info box */
        .user-box { background:var(--green-light); border-radius:var(--radius-sm); padding:12px 16px; display:flex; align-items:center; gap:12px; margin-bottom:16px; }
        .user-box img { width:40px; height:40px; border-radius:50%; object-fit:cover; }
        .user-box .un { font-weight:700; font-size:14px; color:#1a2319; }
        .user-box .ue { font-size:12px; color:var(--gray-muted); }
    </style>
</head>
<body>
        ...
        <div class="kamp-row">
            ...
            <div style="flex:1">
                ...
                <div class="kamp-stats">
                    <div><div class="ks-lbl">Terkumpul</div><div class="ks-val"><?= rupiah((int)$kamp['terkumpul']) ?></div></div>
                    <div><div class="ks-lbl">Target</div><div class="ks-val" style="color:var(--gray-muted)"><?= rupiah((int)$kamp['target_dana']) ?></div></div>
                    <div><div class="ks-lbl">Progress</div><div class="ks-val"><?= $p ?>%</div></div>
                </div>
            </div>
        </div>
...
function toggleAnon(cb) {
    ...
    if (cb.checked) { inp.value='Anonim'; inp.disabled=true; inp.style.opacity='.5'; }
    else { inp.value=''; inp.disabled=false; inp.style.opacity='1'; }
}
function showFile(input) {
    if (input.files[0]) document.getElementById('fileName').textContent = '✓ ' + input.files[0].name;
}
function selectMetode(el, val) {
    document.querySelectorAll('.metode-opt').forEach(function(o){ o.classList.remove('selected'); });
